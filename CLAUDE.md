...
**`igp/`** is an independent subsystem: headless-browser capture and extraction of a *single* Instagram post (`/p/...`) via GraphQL response sniffing (`igp/capture.py`, `igp/extract.py`, `igp/cookies.py`, `igp/select.py`) — separate from, and not used by, the yt-dlp-based `downloaders/instagram.py` reel downloader. Run via `bin/igp_probe.sh URL` (needs `playwright` + `playwright install chromium` in the `dl_wm` env — not part of `environment.yml`, installed ad hoc 2026-09-06). This is the right tool for multi-image carousel posts specifically: `downloaders/instagram.py` (yt-dlp) treats each carousel item as a video-format lookup and fails with "No video formats found" on image slides, falling back to grabbing only the single OpenGraph cover image via HTML scraping — not the full carousel.
