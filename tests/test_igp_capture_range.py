...
def test_find_post_model_from_reduced_sjs_fixture_extracts_expected_assets_and_video_preference():
    fixture_path = "fixtures/instagram/DWj0dQ4moZ8_reduced_one_response.jsonl"
    captured = [json.loads(line) for line in open(fixture_path, "r", encoding="utf-8") if line.strip()]
    model = find_post_model(captured, "DWj0dQ4moZ8")
    assert model is not None
    assert model["shortcode"] == "DWj0dQ4moZ8"
    ...
    urls = [u for _, u in structured_extract(captured[0]["text"], "DWj0dQ4moZ8")]
    assert urls == [
        "https://instagram.fna.fbcdn.net/1.jpg",
        "https://instagram.fna.fbcdn.net/2.mp4",
        "https://instagram.fna.fbcdn.net/3.jpg",
    ]
